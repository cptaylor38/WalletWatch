...
export const filterExpenses = (expenses) => {
  ...
  const thisMonth = (date) => {
    return moment(date).format('MMMM') === moment(Date.now()).format('MMMM')
      ? true
      : false;
  };

  let recurring = expenses.filter((expense) => expense.monthly);
  let nonRecurring = expenses.filter(
    (expense) => !expense.monthly && thisMonth(expense.date)
  );
...
export const categorize = (recurring, nonRecurring) => {
  ...
  function CategoryListConstructor(
    finances,
    health,
    leisure,
    travel,
  ...
  if (nonRecurring.length > 0)
    categoryHelper(nonRecurring, nonRecurringCategoryList);
  else {
